#!/usr/bin/env bash
#
# Seals the NetBird setup key into netbird-secret.sealed.yaml.
#
...
#
# Usage:  ./seal-netbird-secret.sh
#
# Get a *reusable* setup key from https://netbird.arec.me first. Reusable
# matters: the sidecar's state is an emptyDir, so the peer re-registers with
# this key every restart, and a one-off key would work exactly once.

set -euo pipefail

NAMESPACE="netbird-l2tp-bridge"
SECRET_NAME="netbird-l2tp-bridge-netbird"
CONTROLLER_NAME="sealed-secrets-controller"
CONTROLLER_NS="sealed-secrets"
OUT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/netbird-secret.sealed.yaml"
...
printf 'NetBird setup key (input hidden): '
read -rs SETUP_KEY
printf '\n'

[ -n "$SETUP_KEY" ] || { echo "error: empty setup key, nothing to seal." >&2; exit 1; }
...
kubectl create secret generic "$SECRET_NAME" \
  --namespace "$NAMESPACE" \
  --from-literal=NB_SETUP_KEY="$SETUP_KEY" \
...
echo
echo "Next:"
echo "  git add $(basename "$OUT") && git commit -m 'Add sealed NetBird setup key' && git push"
echo
echo "ArgoCD syncs it, the controller unseals it into the $SECRET_NAME Secret,"
echo "and the netbird sidecar picks it up via envFrom. Then add the routes in"
echo "the dashboard (README: 'Advertising the routes') -- sealing the key"
echo "registers the peer but does not advertise anything by itself."
